...
void ADroid::DamageDroid_Implementation(int Amount, FHitResult Hit, AActiveGameState *InstigatorGameState) {
	...
	if (CurrentHealth == 0 && !DroidState.Dead) {
		...
		if (HasAuthority()) {
			...
			if (InstigatorGameState == nullptr) { return; }
			//InstigatorGameState->Points += FMath::Clamp(Amount, 0, CurrentHealth);
		}
